...
def store_post(content):
  source_info = source
  a_tag = content.find_all('a', attrs={'rel':'bookmark'})[0]
...
def process_post(url):
  ...
  if page_content[0]:
    ...
    if len(next_page)==0:
      ...
      container = page_content.find_all('div', attrs={'id':'content'})[0]
      for post in container.find_all('article', attrs={'class':'post'}):
        store_post(post)
